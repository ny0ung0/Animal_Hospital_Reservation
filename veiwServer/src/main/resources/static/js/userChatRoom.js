...
function loadChatHistory() {
    $.ajax({
        ...
        success: function(response) {
            ...
            // 읽음 처리 업데이트
            updateMessagesAsRead(chatMessages);
        },
...
function updateMessagesAsRead(messages) {
    const unreadMessages = messages.filter(chat => chat.isRead === false && chat.sender.id != MemberId);
    const messageIds = unreadMessages.map(chat => chat.id); // 서버에서 메시지를 식별할 수 있는 ID

    if (messageIds.length > 0) {
        $.ajax({
            url: 'http://localhost:9001/chat/updateReadStatus',
            type: 'POST',
            data: JSON.stringify({ messageIds: messageIds, isRead: true }),
            ...
            success: function(response) {
                console.log('메시지 읽음 처리 업데이트 성공:', response);
                // 로컬 스토리지 업데이트
                const updatedMessages = messages.map(chat => {
                    if (messageIds.includes(chat.id)) {
                        chat.isRead = true;
                    }
                    return chat;
                });
            ...
            error: function(xhr, status, error) {
                console.error('메시지 읽음 처리 업데이트 에러발생', error);
            }
        });
    }
}
...
function updateMessageAsRead(message) {
    $.ajax({
        url: 'http://localhost:9001/chat/updateReadStatus',
        type: 'POST',
        data: JSON.stringify({
            messageId: message.id, // 메시지 ID를 사용하여 업데이트
            isRead: true
        }),
        contentType: 'application/json',
        headers: {
